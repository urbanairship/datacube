...
//        }
    ...
    /**
     * Fetches the preaggregated Slice-values from the Cube
     * @param at address of the slice
     * @param bucketer bucketer used
     * @param btype bucketType used
    ...
    public <K> Optional<Map<K, T>> getSlice(ReadBuilder at, Bucketer<K> bucketer, BucketType btype) {
        ...
        Map<K, T> castedMap = new HashMap<K, T>();
        for(Map.Entry<BoxedByteArray, T> entry : optional.get().entrySet()) {
            K convertedKey = bucketer.readBucket(entry.getKey(), btype);
            T val = entry.getValue();

            castedMap.put(convertedKey, val);
        }
